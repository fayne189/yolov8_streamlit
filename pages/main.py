    ...
    if converted_video_path:
        ...
        with final_result_ui:
            ...
            if st.button('predict video'):
                ...
                # def classfiy_callback(predictor):
                #     # Retrieve the batch data
                #     _, im0s, _, _ = predictor.batch
                #     bbox_results = predictor.results
                #     classify_results = []
                #     for i, res in enumerate(bbox_results):
                #         im = im0s[i]
                #         crops = [save_one_box(xyxy, im, save=False, BGR=True) for xyxy in res.boxes.xyxy]
                #         classify_results.append(classify_model(crops))
                #     predictor.results = bbox_results, classify_results
